...
async function* resolvePathConflictsInPassEntryMap(baseHostToPassEntryMap) {
  ...
  for (const [baseHost, passEntries] of baseHostToPassEntryMap.entries()) {
    ...
    for (const passEntry of passEntries) {
      console.log(`\tWorking on passEntry ${passEntry.host} ${passEntry.login}`);
      const entriesWithSameHost = passEntries.filter((entry) => entry.host === passEntry.host);

      let basePath = `${baseHost}/${passEntry.host}`;
      // No need to have redundant folders if the base host is the same as the entry
      // host
      if (baseHost === passEntry.host) {
        basePath = `${baseHost}`;
      }
      ...
      if (entriesWithSameHost.length > 1) {
        ...
        while ((pathConflict = pathToPassEntryMap.has(path)) || !alias || alias === '') {
          console.log(`\tpath=${path} basePath=${basePath} alias=${alias}`);
