...
const { parse: tldUrlParse } = require('tldjs');

// Get the base host/port combo unique to this URL
//
// Related to Fully Qualified Domain Name (FQDN) but also accounts for valid hostnames
// like `localhost` which are invalid TLDs. And includes port number.
//
// Given `https://account.nvidia.com/en-US` -> nvidia.com (also see tests below)
//
// Related supplemental dev notes: https://publicsuffix.org/
function getBaseHost(url) {
  const { host, port } = new URL(url);
  // TLD.js handles the complex logic of figuring out the domain and TLD in cases like
  // `ebay.com.au` where the TLD is `.com.au`.
  const parsedUrl = tldUrlParse(url);

  if (parsedUrl.isIp) {
    // We use the `host` from the URL object because it includes the port number and
    // brackets for IPv6 addresses that are necessary when using in a URL
    // (`[1fff:0:a88:85a3::ac1f]:8001`).
    return host;
  }

  // Technically, a FQDN is this whole piece `mail.example.com` where:
  //
  //  - sub-domain = `mail`
  //  - domain = `example`
  //  - top-level domain = `.com`
  //
  // But for our purposes, we just want the `domain + tld` (e.g. `example.com`)
  //
  // Also, not every hostname is a valid TLD (e.g. `localhost`), so we use the
  // `hostname` as a fallback.
  const baseHost = parsedUrl.domain || parsedUrl.hostname;

  // Append the port number as necessary
...
if (process.env.NODE_TEST_CONTEXT) {
  test('getBaseHost', () => {
    ...
    assert.strictEqual(getBaseHost('https://signin.ebay.com.au/ws/eBayISAPI.dll'), 'ebay.com.au');
    ...
    assert.strictEqual(getBaseHost('http://127.0.0.1:8000/settings/'), '127.0.0.1:8000');
    assert.strictEqual(getBaseHost('http://gdk.test:3000/users/sign_inn'), 'gdk.test:3000');
    assert.strictEqual(
      getBaseHost('http://[1fff:0:a88:85a3::ac1f]/index.html'),
      '[1fff:0:a88:85a3::ac1f]',
    );
    assert.strictEqual(
      getBaseHost('http://[1fff:0:a88:85a3::ac1f]:8001/index.html'),
      '[1fff:0:a88:85a3::ac1f]:8001',
    );
  });
}
...
function printLoginAliasMap(loginAliasMap) {
  ...
  //
  // We print the login alias map to stderr so it doesn't mess with our actual
...
async function interactiveResolvePathConflictsInPassEntryMap(
  ...
  const readline = require('node:readline').createInterface({
    ...
    // stdout is for data, stderr is for commentary
    //
    // We print our user interactions to stderr so it doesn't mess with our actual data
    // output.
